  ...
  return (
    <div className="p-3.5 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight">Welcome, {user?.name || 'Employee'}!</h2>
          <p className="text-xs sm:text-sm text-muted-foreground">Here's your attendance and activity summary for today.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          {status === 'Checked Out' && (
            <Button onClick={handleClockIn} disabled={loading} className="gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs sm:text-sm h-9 sm:h-10 px-4 rounded-xl shadow-xs">
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogIn className="h-4 w-4" />}
              Clock In
            </Button>
          )}
          {status === 'Checked In' && (
            <Button onClick={handleClockOut} disabled={loading} className="gap-2 bg-red-600 hover:bg-red-700 text-white font-semibold text-xs sm:text-sm h-9 sm:h-10 px-4 rounded-xl shadow-xs">
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogOut className="h-4 w-4" />}
              Clock Out
            </Button>
          )}

          <Badge variant="secondary" className={`flex items-center gap-2 px-3 py-1 text-xs font-semibold rounded-full w-fit ${status === 'Checked In' ? 'bg-green-100 text-green-700 dark:bg-green-950/60 dark:text-green-400' :
